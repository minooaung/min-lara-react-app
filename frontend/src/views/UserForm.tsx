...
export default function UserForm(): JSX.Element {
  ...
  const validationErrors = (userError || createUserMutation.error || updateUserMutation.error) as ValidationErrors | null;
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            <div>
              ...
              <input
                id="name"
                value={user.name}
                onChange={(ev: ChangeEvent<HTMLInputElement>) => setUser({ ...user, name: ev.target.value })}
            ...
            <div>
              ...
              <input
                id="email"
                value={user.email}
                onChange={(ev: ChangeEvent<HTMLInputElement>) => setUser({ ...user, email: ev.target.value })}
            ...
            <div>
              ...
              <select
                id="role"
                value={user.role}
                onChange={(ev: ChangeEvent<HTMLSelectElement>) => setUser({ ...user, role: ev.target.value as UserRole | '' })}
                required={!user.id}
            ...
            <div>
              ...
              <input
                id="password"
                type="password"
                onChange={(ev: ChangeEvent<HTMLInputElement>) => setUser({ ...user, password: ev.target.value })}
                placeholder={user.id ? "Leave empty to keep current password" : "Enter password"}
                required={!user.id}
            ...
            <div>
              ...
              <input
                ...
                type="password"
                onChange={(ev: ChangeEvent<HTMLInputElement>) => setUser({ ...user, password_confirmation: ev.target.value })}
                placeholder={user.id ? "Leave empty to keep current password" : "Confirm password"}
                required={!user.id}
                autoComplete="new-password"
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
              />
